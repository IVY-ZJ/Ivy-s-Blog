...
- **侧边栏导航**：首页 / 全部文章 / 学习笔记 / 资源分享 / 标签 / 音乐 / 关于我，移动端折叠为抽屉
- **可拖动侧边栏**：桌面端拖动侧边栏右缘可自由调整宽度（220–420px），偏好会被记住
- **标签系统**：「标签」页按主题聚合文章，点击标签即可筛选
- **站内搜索 + 分类筛选**：「全部文章」页可按关键词与分类过滤，「资源分享」页可搜索资源
...
│  ├─ img/             # 图片素材（壁纸 + 小鸟头像 avatar*.jpg + 封面）
│  ├─ audio/           # 音频（可按需新建）
│  ├─ video/           # 视频（可按需新建）
│  ├─ files/           # 可下载的小文件（可按需新建）
│  └─ js/
│     ├─ posts-data.js # 文章元数据（唯一数据源，含 tags 标签）
│     └─ app.js        # 交互（侧边栏/主题/搜索/标签/拖动调宽/渲染）
