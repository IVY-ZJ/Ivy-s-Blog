...
├─ resources.html      # 资源分享（网盘卡片）
├─ about.html          # 关于我
├─ assets/
│  ├─ css/style.css    # 设计系统（所有样式）
│  ├─ img/             # 图片素材（含壁纸 wallpaper-light/dark.webp）
│  ├─ audio/           # 音频（可按需新建）
│  ├─ video/           # 视频（可按需新建）
│  ├─ files/           # 可下载的小文件（可按需新建）
│  └─ js/
│     ├─ posts-data.js # 文章元数据（唯一数据源）
│     └─ app.js        # 交互（侧边栏/主题/搜索/渲染）
